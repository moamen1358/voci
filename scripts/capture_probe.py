...
def main() -> int:
    ...
    cap = AudioCapture(
        monitor_source=cfg.monitor_source, sample_rate=cfg.sample_rate, block_seconds=0.25
    )
    cap.start()
    time.sleep(0.6)
    ...
    if not chunks:
        print(
            "ERROR: no audio captured. Is parec running, monitor source live, audio playing?",
            file=sys.stderr,
        )
        return 3
